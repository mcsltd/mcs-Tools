...
def main(
    ...
    c = Canvas(f"{to_save}/output.pdf", pagesize=A3)
    ...
    c.save()
    # save dxf file
    new_doc.saveas(f"{to_save}/output.dxf")

    print(f"Программа успешно отработала.\n"
          f"Сгенерированы 2 файла \"output.dxf\" и \"output.pdf\" в папку {to_save}.")
...
if __name__ == "__main__":
    ...
    to_save = f"output_{datetime.now().isoformat().replace(':', '-')[:-7]}"
    os.mkdir(path=f"{to_save}")
